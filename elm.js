...
Elm.CatalogList = Elm.CatalogList || {};
Elm.CatalogList.make = function (_elm) {
   "use strict";
   _elm.CatalogList = _elm.CatalogList || {};
   if (_elm.CatalogList.values) return _elm.CatalogList.values;
   var _U = Elm.Native.Utils.make(_elm),
   $Basics = Elm.Basics.make(_elm),
   $Debug = Elm.Debug.make(_elm),
   $Html = Elm.Html.make(_elm),
   ...
   var thumbnail = function (item) {
      return A2($Html.div,
      _U.list([$Html$Attributes.$class("thumbnail")]),
      _U.list([A2($Html.h5,_U.list([$Html$Attributes.$class("text-center")]),_U.list([$Html.text(item.title)]))
              ,A2($Html.img,_U.list([$Html$Attributes.src(item.image),$Html$Attributes.alt("image")]),_U.list([]))
              ,A2($Html.button,_U.list([$Html$Attributes.$class("btn btn-primary btn-block")]),_U.list([$Html.text("More")]))]));
   };
   var catalogList = function (listings) {
      var thumbnails = A2($List.map,
      function (i) {
         return A2($Html.div,_U.list([$Html$Attributes.$class("col-md-3 col-sm-4 col-xs-12")]),_U.list([thumbnail(i)]));
      },
      listings);
      return A2($Html.div,_U.list([$Html$Attributes.$class("row")]),thumbnails);
   };
   return _elm.CatalogList.values = {_op: _op,thumbnail: thumbnail,catalogList: catalogList};
};
Elm.Details = Elm.Details || {};
Elm.Details.make = function (_elm) {
   "use strict";
   _elm.Details = _elm.Details || {};
   if (_elm.Details.values) return _elm.Details.values;
   var _U = Elm.Native.Utils.make(_elm),
   $Basics = Elm.Basics.make(_elm),
   $Debug = Elm.Debug.make(_elm),
   $Html = Elm.Html.make(_elm),
   ...
   var itemDetails = A2($Html.div,
   _U.list([$Html$Attributes.$class("row")]),
   _U.list([A2($Html.div,
           _U.list([$Html$Attributes.$class("col-md-6")]),
           _U.list([A2($Html.img,_U.list([$Html$Attributes.src("images/ddl.png"),$Html$Attributes.alt("image")]),_U.list([]))]))
           ,A2($Html.div,_U.list([$Html$Attributes.$class("col-md-6")]),_U.list([$Markdown.toHtml(markdown)]))]));
   return _elm.Details.values = {_op: _op,itemDetails: itemDetails,markdown: markdown};
};
Elm.Utils = Elm.Utils || {};
Elm.Utils.make = function (_elm) {
   ...
   var stylesheetLink = function (url) {
      ...
      "link",
      _U.list([A2($Html$Attributes.property,"rel",$Json$Encode.string("stylesheet"))
              ,A2($Html$Attributes.property,"type",$Json$Encode.string("text/css"))
              ,A2($Html$Attributes.property,"href",$Json$Encode.string(url))]),
      _U.list([]));
   };
   return _elm.Utils.values = {_op: _op,stylesheetLink: stylesheetLink};
};
Elm.Main = Elm.Main || {};
Elm.Main.make = function (_elm) {
   ...
   var _U = Elm.Native.Utils.make(_elm),
   $Basics = Elm.Basics.make(_elm),
   $CatalogList = Elm.CatalogList.make(_elm),
   $Debug = Elm.Debug.make(_elm),
   $Details = Elm.Details.make(_elm),
   $Html = Elm.Html.make(_elm),
   $Html$Attributes = Elm.Html.Attributes.make(_elm),
   $List = Elm.List.make(_elm),
   $Maybe = Elm.Maybe.make(_elm),
   ...
   _U.list([$Html$Attributes.$class("row")]),
   _U.list([A2($Html.div,_U.list([$Html$Attributes.$class("col-md-3 col-lg-3")]),_U.list([A2($Html.h1,_U.list([]),_U.list([$Html.text("Catalog")]))]))
           ,A2($Html.div,
           _U.list([$Html$Attributes.$class("col-md-9 col-lg-9")]),
           _U.list([A2($Html.input,
           _U.list([$Html$Attributes.type$("text"),$Html$Attributes.$class("form-control col-md-8 col-lg-4"),$Html$Attributes.placeholder("Search")]),
           _U.list([]))]))]));
   var view = F2(function (address,model) {
      return A2($Html.div,
      _U.list([$Html$Attributes.$class(" catalog container")]),
      _U.list([$Utils.stylesheetLink("https://maxcdn.bootstrapcdn.com/bootstrap/3.3.5/css/bootstrap.min.css")
              ,$Utils.stylesheetLink("styles/main.css")
              ,header
              ,$CatalogList.catalogList(model.catalogList)
              ,$Details.itemDetails]));
   });
   var update = F2(function (action,model) {    var _p0 = action;return model;});
   var Details = function (a) {    return {ctor: "Details",_0: a};};
   var initialModel = {catalogList: _U.list([{id: 1,title: "Some component Title",image: "images/ddl.png"}
   ...
   var Model = function (a) {    return {catalogList: a};};
   var CatalogItems = F3(function (a,b,c) {    return {id: a,title: b,image: c};});
